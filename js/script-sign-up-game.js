...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVjb3RwY2dpc29wdWpleHlkbGh2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDU4NjU5ODYsImV4cCI6MjA2MTQ0MTk4Nn0.OX75S76YAwlqJmXRKZA-udTr1LivfZGftPiUm6zfeDo';
 ...
 initialForm.addEventListener("submit", async (event) => {
   ...
   const gpa = parseFloat(initialForm.gpa.value);
 
   const messageDiv = document.getElementById("password-msg");
 
   const { error } = await supabaseClient.from("first_rege").insert([{
     name,
     phone_id,
     universityName,
     facultyName,
 ...
 signupForm.addEventListener("submit", async (event) => {
   ...
   const phone_id = signupForm.phone.value;
   const email = signupForm.email.value;
   const govern = signupForm.govern.value;
   const password = signupForm.password.value;
   const confirmPassword = signupForm["confirm-password"].value;
   const residence = signupForm.residence.value;
   const birthDate = signupForm.date.value;
  ...
  const percentages = quizResults.percentages;
 
   const { error } = await supabaseClient.from("second_rege").insert([{
     phone_id,
     name,
     email,
     govern,
     ...
     residence,
     birthDate,
     primary_character: quizResults.primaryCharacterKey,
     percent_a: percentages.A,
     percent_b: percentages.B,
     percent_c: percentages.C,
     percent_d: percentages.D
